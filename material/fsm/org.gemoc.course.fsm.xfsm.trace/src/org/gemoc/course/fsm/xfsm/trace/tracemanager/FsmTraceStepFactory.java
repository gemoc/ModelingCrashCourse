...
public class FsmTraceStepFactory implements IStepFactory {
	...
	public fr.inria.diverse.trace.commons.model.trace.Step createStep(
		...
		if (mse.getAction().getName().equalsIgnoreCase("initializeModel")
				&& (ec.getClassifierID() == org.gemoc.course.fsm.xfsm.fsm.FsmPackage.eINSTANCE.getFiniteStateMachine()
						.getClassifierID()))

		{
			step = fsmTrace.Steps.StepsFactory.eINSTANCE.createFsm_FiniteStateMachine_InitializeModel();
		} else

		if (mse.getAction().getName().equalsIgnoreCase("step") && (ec
				.getClassifierID() == org.gemoc.course.fsm.xfsm.fsm.FsmPackage.eINSTANCE.getState().getClassifierID()))
		...
		if (mse.getAction().getName().equalsIgnoreCase("fire")
				&& (ec.getClassifierID() == org.gemoc.course.fsm.xfsm.fsm.FsmPackage.eINSTANCE.getTransition()
						.getClassifierID()))

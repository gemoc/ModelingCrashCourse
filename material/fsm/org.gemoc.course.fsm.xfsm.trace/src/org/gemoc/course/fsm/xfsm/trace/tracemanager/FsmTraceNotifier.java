		...
		if (!changes.isEmpty()) {
			...
			changes.forEach(c -> {
				if (c instanceof NewObjectModelChange) {
					...
					} else if (o instanceof fsmTrace.States.fsm.TracedFiniteStateMachine) {
						newDimensions
								.add(((fsmTrace.States.fsm.TracedFiniteStateMachine) o).getConsummedStringSequence());
						newDimensions.add(((fsmTrace.States.fsm.TracedFiniteStateMachine) o).getCurrentStateSequence());
						newDimensions
								.add(((fsmTrace.States.fsm.TracedFiniteStateMachine) o).getProducedStringSequence());
						newDimensions
								.add(((fsmTrace.States.fsm.TracedFiniteStateMachine) o).getUnprocessedStringSequence());
					}
				}
			});

...
public class FsmTraceExplorer implements ITraceExplorer {
	...
	private List<List<? extends fsmTrace.States.Value>> getAllValueTraces() {
		...
		for (fsmTrace.States.fsm.TracedFiniteStateMachine tracedObject : traceRoot.getFsm_tracedFiniteStateMachines()) {
	...
	private void goTo(EObject eObject) {
		if (eObject instanceof fsmTrace.States.State) {
			...
			for (fsmTrace.States.FiniteStateMachine_consummedString_Value value : stateToGo
					.getFiniteStateMachine_consummedString_Values()) {
				if (value.getParent() instanceof fsmTrace.States.fsm.TracedFiniteStateMachine) {
					fsmTrace.States.fsm.TracedFiniteStateMachine parent_cast = (fsmTrace.States.fsm.TracedFiniteStateMachine) value
							.getParent();
					java.lang.String toset = value.getConsummedString();
					java.lang.String current = ((org.gemoc.course.fsm.xfsm.fsm.FiniteStateMachine) parent_cast
							.getOriginalObject()).getConsummedString();
					if (current != toset) {
						((org.gemoc.course.fsm.xfsm.fsm.FiniteStateMachine) parent_cast.getOriginalObject())
								.setConsummedString((java.lang.String) toset);
					}
				}
			}
			for (fsmTrace.States.FiniteStateMachine_currentState_Value value : stateToGo
					.getFiniteStateMachine_currentState_Values()) {
				if (value.getParent() instanceof fsmTrace.States.fsm.TracedFiniteStateMachine) {
					fsmTrace.States.fsm.TracedFiniteStateMachine parent_cast = (fsmTrace.States.fsm.TracedFiniteStateMachine) value
							.getParent();
					org.gemoc.course.fsm.xfsm.fsm.State toset = value.getCurrentState();
					org.gemoc.course.fsm.xfsm.fsm.State current = ((org.gemoc.course.fsm.xfsm.fsm.FiniteStateMachine) parent_cast
							.getOriginalObject()).getCurrentState();
					if (current != toset) {
						((org.gemoc.course.fsm.xfsm.fsm.FiniteStateMachine) parent_cast.getOriginalObject())
								.setCurrentState((org.gemoc.course.fsm.xfsm.fsm.State) toset);
					}
				}
			}
			for (fsmTrace.States.FiniteStateMachine_producedString_Value value : stateToGo
					.getFiniteStateMachine_producedString_Values()) {
				if (value.getParent() instanceof fsmTrace.States.fsm.TracedFiniteStateMachine) {
					fsmTrace.States.fsm.TracedFiniteStateMachine parent_cast = (fsmTrace.States.fsm.TracedFiniteStateMachine) value
							.getParent();
			...
			for (fsmTrace.States.FiniteStateMachine_unprocessedString_Value value : stateToGo
					.getFiniteStateMachine_unprocessedString_Values()) {
				if (value.getParent() instanceof fsmTrace.States.fsm.TracedFiniteStateMachine) {
					fsmTrace.States.fsm.TracedFiniteStateMachine parent_cast = (fsmTrace.States.fsm.TracedFiniteStateMachine) value
							.getParent();
